# -*- coding: utf-8 -*-
"""
ACL 2026 全量论文 · 中文六维度分析（全量版）
==============================================

读 ACL2026_all_papers.json，调 LLM 给每篇生成中文六维度分析，
输出 ACL2026_all_papers_CN.json。
...
OUTPUT_JSON = os.environ.get("OUTPUT_JSON", "ACL2026_all_papers_CN.json")
REUSE_FROM = os.environ.get("REUSE_FROM", "")
...
USER_PROMPT_TEMPLATE = """请阅读下面这篇 ACL 2026 的论文摘要，并用中文按六个维度进行精炼分析。
...
def main():
    ...
    # 2b. 可选：复用已有翻译，省钱
    n_reused = 0
    if REUSE_FROM and Path(REUSE_FROM).exists():
        try:
    ...
    print(f"\n[OK] 完成！结果保存至 {OUTPUT_JSON}")
    print(f"  成功: {n_ok}  失败: {n_fail}")
    if n_fail > 0:
        print(f"  提示：失败的 {n_fail} 篇可以直接重跑 python translate_all_papers.py，会自动只重做这些。")
