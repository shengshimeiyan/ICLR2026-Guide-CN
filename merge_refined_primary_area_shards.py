# -*- coding: utf-8 -*-
"""Merge GitHub Actions primary-area refinement shard outputs."""
...
from refine_primary_areas import SUBCATEGORY_TAXONOMY


INPUT_JSON = os.environ.get("INPUT_JSON", "ACL2026_all_papers.json")
SHARD_GLOB = os.environ.get("SHARD_GLOB", "ACL2026_refined_primary_areas_shard_*.json")
OUTPUT_JSON = os.environ.get("OUTPUT_JSON", "ACL2026_refined_primary_areas_full.json")
REPORT_MD = os.environ.get("REPORT_MD", "ACL2026_refined_primary_areas_full_report.md")
...
def has_valid_primary_category(record):
...
def write_report(path, summary, category_counts, primary_counts):
    lines = [
        "# ACL2026 Refined Primary Area Merge Report",
        "",
    ...
    lines.extend(f"- {category}: {count}" for category, count in category_counts.most_common(100))
    if summary["missing"]:
    ...
    if summary["error_ids"]:
        lines.extend(["", "## Refinement Error IDs"])
...
def merge_refined_shards(source_json, shard_glob, output_json, report_md, limit_papers=0, stratified=False):
    ...
    invalid = [
        paper["id"]
        for paper in base_papers
        if paper["id"] in refined_by_id and not has_valid_primary_category(refined_by_id[paper["id"]])
    ]
    ...
    if missing or invalid or error_ids or duplicate_ids:
        return summary
    ...
    output = {
        "meta": {
            **source.get("meta", {}),
            ...
            "category_merge_source": "GitHub Actions primary-area shards",
...
def main():
    ...
    if summary["missing"] or summary["invalid"] or summary["error_ids"] or summary["duplicate_ids"]:
        sys.exit(1)
